...
for img_path, img_name in zip(imgs_path, imgs_name):
    ...
    x_img = np.array(img)
    y_img = one_hot_encoder(img_name.split('_')[0], chars) # LOL HO SBAGLIATO TUTTO IL TEMPO L'ENCODING 
    X.append(x_img)
    Y.append(y_img)
...
cnn_model = character_classifier.CharacterRecognitionCNN(num_classes=len(chars))
cnn_model.compile()
cnn_model.train(X_train, Y_train, X_val, Y_val, batch_size=64)
